...
class TIMUIKitGroup extends StatefulWidget {
  ...
  const TIMUIKitGroup({Key? key, this.onTapItem, this.emptyBuilder, this.itemBuilder, this.groupCollector}) : super(key: key);
...
class _TIMUIKitGroupState extends TIMUIKitState<TIMUIKitGroup> {
  ...
  Widget _itemBuilder(BuildContext context, V2TimGroupInfo groupInfo) {
    ...
    return Container(
      decoration: BoxDecoration(border: Border(bottom: BorderSide(color: theme.weakDividerColor ?? CommonColor.weakDividerColor))),
      child: Material(
        ...
        child: InkWell(
          onTap: (() async {
            if (widget.onTapItem != null) {
              ...
              final res = await TencentImSDKPlugin.v2TIMManager.getConversationManager().getConversation(conversationID: "group_${groupInfo.groupID}");
